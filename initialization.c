/*
 * initialization.c: Vehicular Container Placement Problem - Initialization Stage
 * Date: 01-12-2019
 ...
 * parameter: containers requirements matrix
...
int** initialization(int number_of_individuals, int h_size, int v_size, int **V, int max_SLA)
{
	...
	for (iterator_individual=0; iterator_individual < number_of_individuals; iterator_individual++)
	{
		...
		for (iterator_individual_position = 0; iterator_individual_position < v_size; iterator_individual_position++)
		{
			...
			{
				/* assign to each container a random generated physical machine */
...
{
	/* assign to each container a random generated physical machine from 0 to the maximum possible */
	if (SLA == 0)
	{
		/* integer from 0 to max_posible + 1*/
		return rand() % (max_posible + 1);
	}
	/* assign to each container a random generated physical machine from 1 to the maximum possible */
	if (SLA == 1)
	{

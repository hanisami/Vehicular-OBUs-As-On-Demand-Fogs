/*
 * pareto.c: Vehicular Container Placement Problem - Pareto Functions
 * Date: 01-12-2019
...
void getHighestFitnessSolution(struct pareto_element *pareto_head, int **H, int **V, int v_size, int h_size) {
	...
	int iterator_individual;
	int iterator_virtual;
	float highestFitnessSolution = 0.0;
	int solutionIndex;
	/* copying pareto individuals */
	for (iterator_individual=0; iterator_individual < pareto_size; iterator_individual++)
	{
		best_P[iterator_individual] = (int *) malloc (v_size *sizeof (int));
		objective_functions_best_P[iterator_individual] = (float *) malloc (4 *sizeof (float));
		best_P[iterator_individual] = pareto_head->solution;
		objective_functions_best_P[iterator_individual] = pareto_head->costs;
		pareto_head = pareto_head->next;
	}
	for (iterator_individual=0; iterator_individual < pareto_size; iterator_individual++){
		...
		float objective1 = (float) objective_functions_best_P[iterator_individual][0];
		float objective2 = (float) objective_functions_best_P[iterator_individual][1];
		float objective3 = (float) objective_functions_best_P[iterator_individual][2];
		float objective4 = (float) objective_functions_best_P[iterator_individual][3];
		float currentFitness = (0.25 * objective1) - (0.25 * objective2) + (0.25 * objective3) - (0.25 * objective4);
		if (currentFitness > highestFitnessSolution){
			highestFitnessSolution = currentFitness;
			solutionIndex = iterator_individual;
		}
	}
	/*print best solution for current generation*/
	// for (iterator_virtual = 0; iterator_virtual < v_size ; iterator_virtual++)
	// {
	// 	printf("%d\t",best_P[solutionIndex][iterator_virtual]);
	// }
	printf("%f\n", highestFitnessSolution);
}
